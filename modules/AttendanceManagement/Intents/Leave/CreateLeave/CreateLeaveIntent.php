...
class CreateLeaveIntent
{
    ...
    public function handle(Request $request)
    {
        ...
        try {
            ...
            if ($createLeaveUserDTO['leave_duration_type'] == "Half-Day" || $createLeaveUserDTO['leave_duration_type'] == "Full-Day") {
                $leaveCount = Leave::where('employee_id', $createLeaveUserDTO['employee_id'])
                    ->where(function($query) use ($createLeaveUserDTO) {
                        $query->where('leave_date', $createLeaveUserDTO['leave_date'])
                            ->orWhere('leave_period_start_date', $createLeaveUserDTO['leave_date'])
                            ->orWhere('leave_period_end_date', $createLeaveUserDTO['leave_date'])
                            ->orWhere(function($subQuery) use ($createLeaveUserDTO) {
                                $subQuery->where('leave_period_start_date', '<=', $createLeaveUserDTO['leave_date'])
                                        ->where('leave_period_end_date', '>=', $createLeaveUserDTO['leave_date']);
                            });
                    })
                    ->count();
            }
                ...
                $leaveCount = Leave::where('employee_id', $createLeaveUserDTO['employee_id'])
                    ->where(function($query) use ($createLeaveUserDTO) {
                        $query->where('leave_date', $createLeaveUserDTO['leave_period_start_date'])
                            ->orWhere('leave_date', $createLeaveUserDTO['leave_period_end_date'])
                            // Overlapping periods check
                            ->orWhereBetween('leave_period_start_date', [
                                $createLeaveUserDTO['leave_period_start_date'], 
                                $createLeaveUserDTO['leave_period_end_date']
                            ])
                            ->orWhereBetween('leave_period_end_date', [
                                $createLeaveUserDTO['leave_period_start_date'], 
                                $createLeaveUserDTO['leave_period_end_date']
                            ])
                            ->orWhere(function($subQuery) use ($createLeaveUserDTO) {
                                $subQuery->where('leave_period_start_date', '<=', $createLeaveUserDTO['leave_period_start_date'])
                                        ->where('leave_period_end_date', '>=', $createLeaveUserDTO['leave_period_end_date']);
                            });
                    })
                    ->count();
